...
struct BillListView: View {
        ...
        Menu {
            Menu {
                Button {
                    ...
                    Label("Export Bills", systemImage: "square.and.arrow.up")
                }
                Button {
    ...
    private func exportBills(format: ExportFormat) {
        let bills = normalizedSearchText.isEmpty ? billViewModel.bills : filteredBills
        guard !bills.isEmpty else {
            showExportError("There are no bills to export with the current filters.")
            return
        }
        
        // Deduplicate recurring bills - only export one bill per series
        let deduplicatedBills = deduplicateRecurringBills(bills)
    ...
    private func importBillsFromJSON(data: Data) throws {
        ...
        var skipped = 0
        var errors: [String] = []
        var createdBills: [Bill] = [] // Track created bills for batch save
        var importKeys: Set<String> = [] // Track bills being imported to prevent duplicates within the same import
        ...
        for billDict in billsArray {
            ...
            // Create a unique key for this bill to check for duplicates within the import
            let calendar = Calendar.current
            ...
            // Check if we've already imported this bill in this batch
            if importKeys.contains(importKey) {
                print("⚠️ Skipping duplicate within import: \(name) on \(dueDate)")
                skipped += 1
                continue
            }
            ...
            // Check if bill already exists in database (check both DB and pending)
            if billViewModel.billExists(name: name, dueDate: dueDate, amount: amount) {
                print("⚠️ Skipping duplicate in database: \(name) on \(dueDate)")
                skipped += 1
                continue // Skip duplicate
            }
            
            // Create bill without saving (batch save at end)
            guard let newBill = billViewModel.addBill(
                ...
                skipSave: true // Don't save yet - batch save at end
            ) else {
                print("⚠️ Failed to create bill: \(name) on \(dueDate)")
                skipped += 1
                continue // Bill failed to create
            }
            
            // Mark this bill as imported
            importKeys.insert(importKey)
            
            // If this is a historical paid bill, mark it as paid and create transaction
            if isPaid {
                newBill.isPaid = true
                newBill.paidDate = paidDate ?? dueDate
                
                // Create ledger entry if bill has an account (not credit card only)
        ...
        // Batch save all created bills at once
        if imported > 0 {
            do {
                let context = PersistenceController.shared.container.viewContext
                try context.save()
                print("✅ Batch saved \(imported) imported bills")
            } catch {
            ...
            if skipped > 0 {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                    print("📊 Import complete: \(imported) imported, \(skipped) skipped (duplicates)")
                }
            } else {
                showImportSuccessAlert = true
            }
            // Skip auto-pay processing for 5 seconds after import to prevent processing imported bills
            billViewModel.skipAutoPayProcessing(for: 5.0)
            billViewModel.fetchBills(skipAutoPay: true)
            
            // Run duplicate cleanup immediately after import to catch any that slipped through
    ...
    private func importBillsFromCSV(data: Data) throws {
        ...
        let recurrenceIntervalIndex = header.firstIndex(of: "Recurrence Interval") ?? header.firstIndex(of: "recurrenceInterval") ?? header.firstIndex(of: "Recurrence Interval")
        ...
        for (index, line) in lines.dropFirst().enumerated() {
            ...
            let paidDateIndex = header.firstIndex(of: "Paid Date") ?? header.firstIndex(of: "paidDate") ?? header.firstIndex(of: "Paid Date")
        ...
        if imported > 0 {
            ...
            showImportError("No bills were imported. All bills may already be marked as paid.")
        }
    }
