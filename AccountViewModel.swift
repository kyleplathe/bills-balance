...
extension LedgerEntry {
    ...
    func amountInCurrency(for account: Account) -> Decimal {
        if account.currencyCode == "BTC" {
            // For BTC accounts, use BTC amount if available, otherwise convert USD amount
            if btcAmountDecimal != .zero {
                return btcAmountDecimal
            } else if usdAmountDecimal != .zero && btcPriceAtTransactionDecimal > 0 {
                // Convert USD to BTC using the price at transaction time
                return usdAmountDecimal / btcPriceAtTransactionDecimal
            }
            // Fallback: if no BTC amount stored, return 0 (shouldn't happen for BTC accounts)
            return .zero
        } else {
            // For USD accounts, use USD amount if available, otherwise use regular amount
    ...
    // Returns signed amount in the account's currency
